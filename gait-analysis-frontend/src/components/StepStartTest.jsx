...
const StepStartTest = ({
  ...
  const handleStart = async () => {
    try {
      ...
      const response = await axiosInstance.post("/api/test-sessions", {
        action: "START",
        timestamp,
      });
  ...
  const handleStop = async () => {
    try {
      const timestamp = new Date().toISOString();
      await axiosInstance.put(`/api/test-sessions/${sessionId}`, {
        action: "STOP",
        timestamp,
      });
    ...
    >
      <CardContent sx={{ p: 4 }}>
        ...
        <Grid container spacing={3} sx={{ mb: 4 }}>
          ...
          <Grid item xs={12} sm={4}>
            <DeviceStatusCard
              status={orientationCaptured}
              label="Orientation Captured"
              icon={CheckCircle}
            />
          </Grid>
        </Grid>
