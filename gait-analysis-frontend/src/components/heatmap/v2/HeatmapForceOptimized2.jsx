...
const HeatmapWebSock3Force = ({ sensorData }) => {
  // Canvas refs
  ...
  const CONFIG = {
    ...
    colorThresholds: [
      ...
      { value: 500, color: { r: 0, g: 255, b: 0 } }, // Green
      { value: 1500, color: { r: 255, g: 255, b: 0 } }, // Yellow
      { value: 3000, color: { r: 255, g: 165, b: 0 } }, // Orange
    ],
    ...
    if (sensorData) {
      ...
      const newValues = Array(16)
        ...
        .map((_, index) => {
          const sensorKey = `fsr_${index + 1}`;
          // Convert value to number and handle negative values
          return Math.max(0, Number(sensorData[sensorKey] || 0));
        });
